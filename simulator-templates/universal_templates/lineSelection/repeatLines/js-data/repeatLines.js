...
;(() => {
  ...
  // массив id линий svg, которые нужно обвести согласно примеру
...
<svg
            ...
            >
              ...
              <g id="grid__noactive_" class="repeatLines_st0">
                ...
                <line
                  id="Line15-4_noactive"
                  class="repeatLines_st2"
                  x1="242.3"
                  y1="359.3"
                  x2="359.3"
                  y2="242.3"
...
`
  ...
  renderRepeatLines(taskId, data, Img, svgImg, extraSets)
})()

function renderRepeatLines(taskId, data, Img, svgImg, extraSets) {
  const taskWrapper = document.getElementById(`${taskId}`)
  createTemplateBaseMarkup(taskWrapper, createBaseMarkup)
  const repeatField_img = taskWrapper.querySelector('.repeatField_img')
  if (Img) {
    repeatField_img.innerHTML = `<img src="${Img}" />`
  }
  const lineField = taskWrapper.querySelector('.repeatField')
  let isGameStart = false

  if (svgImg) {
    lineField.innerHTML = `${svgImg}`
  }
  ...
  function createBaseMarkup() {
    return `
  <div class="repeatFieldContainer">
        <div class="repeatField_img"></div>
        <div class="repeatField"></div>
      </div>
      `
  }
}
